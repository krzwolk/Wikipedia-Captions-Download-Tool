...
Article = namedtuple('Article', ('lang', 'id', 'title', 'text'))
...
def article_ids_from_langlinks(file):
    """TODO: Docstring for article_ids_from_langlinks.
    ...
    """
    ids = set()
    for art in parse_lang_links('??', file):
        if ':' not in art.tgt_title:
            ids.add(art.orig_id)
    return ids
